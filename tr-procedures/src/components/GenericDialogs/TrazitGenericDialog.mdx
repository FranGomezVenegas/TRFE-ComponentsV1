...
<Meta title="TRAZiT Actions/Dialogs/Generic Dialog doc" />


        # TRAZiT Generic Dialog

        The \`TrazitGenericDialog\` dialog is a highly adaptative dialog that should be seen as the first solution for any dialog.

        ## Overview

        The \`TrazitGenericDialog\` dialog includes a diversity of possibilities mostly optionals to let use what required for each particular case.
        ...
        - It includes up to 10 clones of each type of field mentioned below. The reason of cloning each type is due to each one will manage its behavior and value as to be referenced to each one without impacting one to the other, at the same time all behaviors and features are ready to use for each clone of.
        
        bla bla bla ... continue from here
        
        ## Lists - Multilists
        - Probably the most rich in features and then complex to set due to the diversity.
        - ways
